    ...
    if (safety_constrain):
        ...
        for k in range(8):
            if ob.opponents[k+3] < safety_distance_lat:
                #action_steer += 0.2
                action_steer -= ob.speedX*2.5*(4-(ob.opponents[k+22] * 200))/4 # used to be 0.5 *
                print("Side collision warning")
                break
    ...
    for i in range(episode_count):
        ...
        for j in range(max_steps):
            ...
            if train_indicator:
                ...
                a_t_original = actor.target_model.predict(s_t.reshape(1, s_t.shape[0]))
            noise_t[0][0] = train_indicator * max(epsilon, 0.1) * OU.function2(a_t_original[0][0],  0.5 , 0.90, 0.2)
            #noise_t[0][1] = train_indicator * max(epsilon, 0.0) * OU.function(a_t_original[0][1],  1.0 , 1.00, 0.10)
            noise_t[0][1] = train_indicator * max(epsilon, 0.1) * OU.function1(a_t_original[0][1],  0.9 , 1.0, 0.60)
        ...
        if train_indicator:
            ...
            sio.savemat('results_overtaking.mat',{'total_reward':cumreward_list,'average_reward':average_step_reward_list,'epsilon':epsilon_list,'damage':damage_rate_list})
        else:
            sio.savemat('info.mat',{'ep_reward':epreward_list,'trackPos':trackPos_list,'speed':speed_list,'damage_rate':damage_rate,'damage_time':damage_time})
